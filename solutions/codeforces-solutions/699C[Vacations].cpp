...
long long go(int i, int lastDay) {
	...
	if(i > n-1) return dp[i][lastDay] = 0;

	// no contest and no gym -> rest and go to next day
	if(days[i] == 0) return dp[i][lastDay] = go(i+1, 0);

	// only contest -> do contest if no contest happened on the last day
	if(days[i] == 1 and lastDay != 1) return dp[i][lastDay] = max(1 + go(i+1, 1), go(i+1, 0));
	else if(days[i] == 1 and lastDay == 1) return dp[i][lastDay] = go(i+1, 0);

	// only gym -> go to the gym if no gym last day
	if(days[i] == 2 and lastDay != 2) return dp[i][lastDay] = max(1 + go(i+1, 2), go(i+1, 0));
	else if(days[i] == 2 and lastDay == 2) return dp[i][lastDay] = go(i+1, 0);
